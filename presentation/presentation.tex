...
\begin{frame}{Model Predictive Control}
  Find sequence, \pause apply first control, \pause rinse repeat \pause $\to$ Receding Horizon

  \begin{figure}
    \centering
    \foreach \i in {1,...,10}{\includegraphics<\i>[width=.5\textwidth]{mpc\i.pdf}%
    }
  \end{figure}
\end{frame}
...
\begin{frame}
  \begin{itemize}
    \item Problem: Complexity depends on $N_{p},m,p$ and sizes of $\vec{x}$ and $\vec{u}$
    \item Solution: Divide and Conquer
  \end{itemize}
\end{frame}
