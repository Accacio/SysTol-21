...
\begin{frame}[plain]
  \titlepage%
  \note{15 minutes !!!!\\}
  \script{Hello, I'm Rafael Accácio and on behalf of my colleagues Romain Bourdais and Hervé Guéguen I'm here to present our paper for the SysTol'21 called «Detection and Mitigation of Corrupted Information in Distributed Model Predictive Control Based on Resource Allocation».\\}
...
\only<5->{
  \begin{minipage}{.5\textwidth}
    \begin{itemize}
      \item<5> Geographically distributed
      \item<6> Coupled by constraints (energy)
      \item<7> Optimization objectives
            \begin{itemize}
              \item Energy
              \item User satisfaction
              \item \dots
            \end{itemize}
      \item<8-> Solution $\to$ Model Predictive Control
    \end{itemize}
  \end{minipage}
  }
  ...
  \script<8>{A solution is to use Model Predictive Control.}
\end{frame}
...
\begin{frame}{Distributed Model Predictive Control}{Quantity Decomposition | Resource Allocation}
  ...
  \script<2->{multiple sub-problems, which can be solved in parallel, and a master problem which corresponds to the initial problem. \\
   Those coupling constraints are replaced by local constraints with an allocation theta i. }
...
\begin{frame}{Outline}
  % \tableofcontents
  \tableofcontents[subsectionstyle=show/show/show,pausesections]
